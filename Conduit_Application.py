...
if selected == "Home":
    ...
    st.write(
        "_**Introduction:** This web application allows you to use machine learning to analyze text. On the left-hand side of the screen you can see six analysis tasks that can be applied to your text._"
    )

    st.write(
        "**Instructions:** The first step is to choose how you would like to input your text. You can either type it into the text box (single document), or upload a spreadsheet with multiple rows of text (multiple documents). Once you have saved and submitted the text, you can proceed to the analysis tasks."
    )

    choice = st.radio(
        "Please select one of the following options:",
        ["Type in Text Box", "Upload Spreadsheet with Text Column"],
    )
    if choice == "Type in Text Box":
        ...
        if submit1:
            if len(text) < 1:
                ...
                st.session_state.multitext_short = ""
                st.write(
                    "Text saved successfully! You are now ready to try out the analysis tasks."
                )

    if choice == "Upload Spreadsheet with Text Column":
        st.write(
            "Below is an example of a properly formatted spreadsheet file. The first column should contain the text you would like analyzed. Additional columns, such as 'Source' and 'Date' in the example below, are optional."
        )
        demo = {
            "Text": [
            ...
            "Source": ["Internet", "Newspaper", "TV"],
            "Date": ["2/13/2021", "4/27/2021", "8/1/2021"],
        }
        demo1 = pd.DataFrame(demo)
        st.dataframe(demo1)
        dfs = st.file_uploader(
            "Please drag and drop your file into the space below (.csv files only)",
            type=["csv"],
        )
        ...
        if submit2:
            if dfs is None:
                ...
                st.write(
                    "Text uploaded successfully! You are now ready to try out the analysis tasks."
                )
...
if selected == "Basic Text Analysis":
    ...
    st.write(
        "**Description:** This task involves producing simple descriptive statistics about your text."
    )
    ...
    if submit3:
        if len(st.session_state.text) > 0:
            ...
            word_freq_df = pd.DataFrame(
                word_freq.most_common(), columns=["Word", "Count"]
            )
            ...
            word_freq_df1 = pd.DataFrame(
                word_freq1.most_common(), columns=["Word", "Count"]
            )
...
if selected == "Named Entity Recognition":
    ...
    st.write(
        "**Description:** This task involves identifying geopolitical entities, organizations, people, and locations in a body of text."
    )
    ...
    if submit4:
        if len(st.session_state.text) > 0:
            ...
            if len(doc_NER.ents) > 0:
                ...
                df14 = pd.DataFrame(
                    {"Entity": entities, "Type of Entity": entityLabels}
                )
                ...
                df14["Type of Entity"] = (
                    ...
                    .str.replace("ORG", "Organization")
                    .str.replace("LOC", "Location")
                    .str.replace("GPE", "Geopolitical Entity")
                )
                df14.sort_values(["Type of Entity"], ascending=False, inplace=True)
                df14 = (
                    df14.groupby(["Type of Entity", "Entity"])
                    .size()
                    .reset_index(name="Count")
                    .sort_values(by=["Count", "Type of Entity"], ascending=False)
                )

                st.write(df14)
                NER_Results1 = df14.to_csv(index=False, header=True)
                st.download_button(
                    "Download Results", NER_Results1, file_name="NER_Results.csv"
                )
            ...
            df3 = (
                df3.groupby(["Type of Entity", "Entity"])
                .size()
                .reset_index(name="Count")
                .sort_values(by=["Count", "Type of Entity"], ascending=False)
            )
            ...
            st.download_button(
                "Download Results", NER_Results, file_name="NER_Results.csv"
            )
...
if selected == "Text Classification":
    ...
    st.write(
        "**Description:** This task involves placing a piece of text into one or more categories."
    )

    cat1 = st.text_input(
        "Enter each possible category name (separated by a comma). Maximum 5 categories."
    )
    ...
    if submit3:
        if len(st.session_state.text) > 0:
            text_class = query(
                {
                    "inputs": st.session_state.text,
                    "parameters": {"candidate_labels": labels1},
                }
            )
            ...
            st.write(
                "**Category:** {} | **Probability:** {:.1f}%".format(
                    cat1name.title(), (cat1prob * 100)
                )
            )
            plot_result(
                text_class["labels"][::-1][-10:], text_class["scores"][::-1][-10:]
            )

        elif len(st.session_state.multitext_all) > 0:
            st.write("--------------------------------------------")
            st.write(
                "_Note: During the current testing phase, this task can only be performed on the first 10 documents._"
            )
            text_class1 = []
            class_name = []
            score_name = []
            ...
            df15 = pd.DataFrame(
                {"Text": text_class1, "Category": class_name, "Probability": score_name}
            )
            st.write(df15)
...
if selected == "Text Summarization":
    ...
    st.write(
        "**Description:** This task involves condensing larger bodies of text into smaller bodies of text."
    )
    API_URL = "https://api-inference.huggingface.co/models/facebook/bart-large-cnn"
    max_lengthy = st.slider(
        "Maximum summary length (words)", min_value=30, max_value=120, value=50, step=5
    )
    min_lengthy = st.slider(
        "Minimum summary length (words)", min_value=10, max_value=60, value=30, step=5
    )
    sum_choice = st.radio(
        "If analyzing multiple texts (i.e. a CSV file), please choose how you would like to summarize your data.",
        ["All documents to one summary.", "Each document summarized individually."],
    )
    ...
    if submit5:
        if len(st.session_state.text) > 0:
            ...
            if sum_choice == "Each document summarized individually.":
                ...
                st.download_button(
                    "Download Results", Summary_Results, file_name="Summary_Results.csv"
                )
        else:
            st.write("Please enter text or upload file on the Home page.")
...
if selected == "Topic Modeling":
    ...
    st.write(
        "**Description:** This task involves finding topics among a collection of documents."
    )
    st.write(
        "_Note: This task only applies to multiple document files (i.e. an uploaded CSV file)._"
    )
    graph_topic_to_highlight = [0, 2, 4]

    num_topics = st.slider(
        "Number of Topics to Find ", min_value=2, max_value=12, value=6, step=1
    )
    words_in_topic = 10
    n_grams = st.radio(
        "N-Gram Size", ["Single Word", "Two Word Phrases", "Three Word Phrases"]
    )
    model_type = st.radio(
    ...
    if submit4:
        if len(st.session_state.text) > 0:
            st.write(
                "I am sorry this method does not apply to single texts. Please return to the Home page and upload a CSV file of mutiple texts."
            )
        elif len(st.session_state.multitext_all) > 0:
            corpus = textacy.Corpus(
                "en_core_web_sm", data=st.session_state.multitext_all
            )
            docs_terms = (
                textacy.extract.terms(
                    ...
                    ngs=partial(
                        ...
                        n=n_grams,
                        include_pos={"PROPN", "NOUN", "ADJ", "VERB"},
                    ),
                )
                for doc in corpus
            )
            ...
            doc_term_matrix, vocab = textacy.representations.build_doc_term_matrix(
                tokenized_docs, tf_type="linear", idf_type="smooth"
            )
            ...
            for topic_idx, terms in model.top_topic_terms(
                id_to_term, top_n=words_in_topic
            ):
                st.write(f"**Topic {topic_idx}**: {'; '.join(terms)}")
            st.write("--------------------------------------------")
            st.write("**Chart of Topics and Words**")
            plot1 = model.termite_plot(
                doc_term_matrix,
                id_to_term,
                n_terms=12,
                highlight_topics=graph_topic_to_highlight,
                save="termite_plot.png",
            )
...
if selected == "Document Clustering":
    ...
    st.write(
        "**Description:** This task involves placing documents into groups based on similarity and then extracting the key words/phrases from each group."
    )
    st.write(
        "_Note: This task only applies to multiple document files (i.e. an uploaded CSV file)._"
    )
    API_URL = (
        "https://api-inference.huggingface.co/models/Craig/paraphrase-MiniLM-L6-v2"
    )
    ensure_model_is_ready = query(
        {"inputs": ["test 1", "test 2"], "parameters": {"wait_for_model": True}}
    )

    num_clusters = st.slider(
        "Number of Clusters to Create ", min_value=2, max_value=10, value=4, step=1
    )
    ...
    if submit6:
        if len(st.session_state.text) > 0:
            ...
            df1 = pd.DataFrame(
                list(zip(list_of_text, cluster_assignment)), columns=["text", "cluster"]
            )
            ...
            for i in lister2:
                keyphrase_data = kw_model.extract_keywords(
                    docs=i,
                    vectorizer=kp_vect,
                    top_n=6,
                    use_maxsum=True,
            ...
            df4 = pd.DataFrame(
                list(zip(lister, joined_list)),
                columns=["Cluster Number", "Key Words/Phrases Within Each Cluster"],
                index=None,
            )
            df4.index = [""] * len(df4)
            st.write(
                "The table below displays the document clusters and key words/phrases extracted from each cluster."
            )
            st.table(df4)
            ...
            st.write(
                "Below is an interactive 2D scatter plot of the documents. Each point represents a document and the colors correspond to the clusters. Click and drag over several points to see the text displayed on the right."
            )

            pipe = make_pipeline(TfidfVectorizer(), UMAP(random_state=42))
            X = pipe.fit_transform(st.session_state.multitext_all)
            temp_chart = plot_text(
                X, st.session_state.multitext_all, color_array=pca_outputs.cluster
            )
            st.altair_chart(temp_chart, use_container_width=True)
            ...
            fig = px.scatter_3d(
                ...
                z="Third Component",
                color="cluster",
                hover_name="short_text",
            )
            st.write(
                'Below is an interactive 3D scatter plot of the documents. Each point represents a document and the colors correspond to the clusters. You can explore the graphic closer by clicking the "view fullscreen" button in the top right corner.'
            )
            st.plotly_chart(fig)
            ...
            st.download_button(
                "Download Results",
                Cluster_Results,
                file_name="Document_Clustering_Results.csv",
            )
